  ...
  return (
    <Card className="py-4 max-w-[750px] flex flex-col gap-3 element border border-transparent hover:border-color-blue p-4 rounded-lg bg-[var(--bg-elements)]">
        <CardHeader className="pb-0 pt-2 px-4 flex-col items-start gap-6">
            ...
            <small className="text-color-bue text-ms md:text-lg lg:text-lg font-bold">{participation}</small>
            <small className="text-ms sm:text-base text-[color:var(--text-color)]">{description}</small>
        </CardHeader>
        ...
        <div className="flex flex-wrap gap-3 p-3">
            {technologies.map((technology, index) => (
                <Code key={index} color="primary" className="text-ms border border-transparent hover:border-color-blue p-2 rounded-lg">{technology}</Code>
            ))}
        </div>
    </Card>
  )
